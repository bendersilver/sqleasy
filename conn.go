...
func (c *Conn) columnBlob(pstmt uintptr, iCol int) (v []byte, err error) {
	p := sqlite.Xsqlite3_column_blob(c.tls, pstmt, int32(iCol))
...
func (c *Conn) columnBytes(pstmt uintptr, iCol int) (_ int, err error) {
	v := sqlite.Xsqlite3_column_bytes(c.tls, pstmt, int32(iCol))
	return int(v), nil
}

func (c *Conn) columnText(pstmt uintptr, iCol int) (v string, err error) {
	p := sqlite.Xsqlite3_column_text(c.tls, pstmt, int32(iCol))
	len, err := c.columnBytes(pstmt, iCol)
	if err != nil {
		return "", err
	}
...
func (c *Conn) retry(pstmt uintptr) error {
	...
	rc := sqlite.Xsqlite3_unlock_notify(
		c.tls,
		c.db,
	...
	mutexFree(c.tls, mu)
	if pstmt != 0 {
		sqlite.Xsqlite3_reset(c.tls, pstmt)
	}
	return nil
}
...
func (c *Conn) bindNull(pstmt uintptr, idx1 int) (uintptr, error) {
	if rc := sqlite.Xsqlite3_bind_null(c.tls, pstmt, int32(idx1)); rc != sqlite.SQLITE_OK {
...
func (c *Conn) bindText(pstmt uintptr, idx1 int, value string) (uintptr, error) {
	...
	if rc := sqlite.Xsqlite3_bind_text(c.tls, pstmt, int32(idx1), p, int32(len(value)), 0); rc != sqlite.SQLITE_OK {
		c.free(p)
		return 0, c.errstr(rc)
	}
...
func (c *Conn) bindBlob(pstmt uintptr, idx1 int, value []byte) (uintptr, error) {
	if value != nil && len(value) == 0 {
		if rc := sqlite.Xsqlite3_bind_zeroblob(c.tls, pstmt, int32(idx1), 0); rc != sqlite.SQLITE_OK {
			return 0, c.errstr(rc)
		}
		return 0, nil
	}
	...
	if rc := sqlite.Xsqlite3_bind_blob(c.tls, pstmt, int32(idx1), p, int32(len(value)), 0); rc != sqlite.SQLITE_OK {
		c.free(p)
		return 0, c.errstr(rc)
	}
...
func (c *Conn) bindInt(pstmt uintptr, idx1, value int) (err error) {
	if rc := sqlite.Xsqlite3_bind_int(c.tls, pstmt, int32(idx1), int32(value)); rc != sqlite.SQLITE_OK {
		return c.errstr(rc)
	}
...
func (c *Conn) bindDouble(pstmt uintptr, idx1 int, value float64) (err error) {
	if rc := sqlite.Xsqlite3_bind_double(c.tls, pstmt, int32(idx1), value); rc != 0 {
		return c.errstr(rc)
	}
...
func (c *Conn) bindInt64(pstmt uintptr, idx1 int, value int64) (err error) {
	if rc := sqlite.Xsqlite3_bind_int64(c.tls, pstmt, int32(idx1), value); rc != sqlite.SQLITE_OK {
		return c.errstr(rc)
	}
...
func (c *Conn) bindParameterName(pstmt uintptr, i int) (string, error) {
	p := sqlite.Xsqlite3_bind_parameter_name(c.tls, pstmt, int32(i))
	return libc.GoString(p), nil
}

func (c *Conn) bindParameterCount(pstmt uintptr) (_ int, err error) {
	r := sqlite.Xsqlite3_bind_parameter_count(c.tls, pstmt)
	return int(r), nil
}

func (c *Conn) finalize(pstmt uintptr) error {
	if rc := sqlite.Xsqlite3_finalize(c.tls, pstmt); rc != sqlite.SQLITE_OK {
		return c.errstr(rc)
	}
...
func (c *Conn) prepareV2(zSQL *uintptr) (pstmt uintptr, err error) {
	...
	for {
		switch rc := sqlite.Xsqlite3_prepare_v2(c.tls, c.db, *zSQL, -1, ppstmt, pptail); rc {
...
func (c *Conn) extendedResultCodes(on bool) error {
	if rc := sqlite.Xsqlite3_extended_result_codes(c.tls, c.db, libc.Bool32(on)); rc != sqlite.SQLITE_OK {
		return c.errstr(rc)
	}
	return nil
}
...
func (c *Conn) openV2(name string, flags int32) (uintptr, error) {
	...
	if rc := sqlite.Xsqlite3_open_v2(c.tls, s, p, flags, 0); rc != sqlite.SQLITE_OK {
		return 0, c.errstr(rc)
	}
...
func (c *Conn) errstr(rc int32) error {
	p := sqlite.Xsqlite3_errstr(c.tls, rc)
	str := libc.GoString(p)
	p = sqlite.Xsqlite3_errmsg(c.tls, c.db)
	var s string
...
func (c *Conn) closeV2(db uintptr) error {
	if rc := sqlite.Xsqlite3_close_v2(c.tls, db); rc != sqlite.SQLITE_OK {
		return c.errstr(rc)
	}
